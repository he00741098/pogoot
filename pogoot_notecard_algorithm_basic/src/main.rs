...
fn main() {
    ...
    } else if multi_file_alt_spanish.is_ok()
        && multi_file_english.is_ok()
        && multi_file_spanish.is_ok()
    {
        ...
        for i in 0..spanish.len() {
            if spanish[i].is_empty() || (english[i].is_empty() && spanish_alt.is_empty()) {
                continue;
            }
            questions.push(question {
                turns_until_repeat: 0,
                corrects: 0,
                wrongs: 0,
                front: spanish[i].to_string(),
                back: format!("{}\n\n{}", english[i], spanish_alt[i]),
            });
        }
        println!("Full questions set: {:?}", questions);
        questions
    } else {
        let result = std::fs::read_to_string("quizlet.txt").unwrap();
        ...
        questions
            ...
            .filter(|x| !x.front.is_empty() && !x.back.is_empty())
...
fn serve_question_short_answer(mut question: &mut question) {
    ...
    if question.front.trim().to_lowercase() == answer.trim().to_lowercase()
        || question
            .front
            .split("/")
            .map(|x| x.trim().to_lowercase())
            .collect::<Vec<String>>()
            .contains(&answer.trim().to_lowercase())
    {
        println!("Correct!");
        question.corrects += 1;
        if question.corrects > question.wrongs {
            question.turns_until_repeat =
                rng.gen_range(20..40 * question.corrects - question.wrongs);
        } else {
            question.turns_until_repeat = rng.gen_range(8..15);
        }
    } else {
        println!(
            "Wrong!: You answered: '{}', Correct answer: '{}'",
            answer,
            question.front.trim()
        );
        question.wrongs += 1;
...
fn serve_question_multiple_choice(index: usize, mut questions: &mut Vec<question>) {
    ...
    if input < other_options.len() && other_options[input] == &question.back {
        ...
        if questions[index].corrects > questions[index].wrongs {
            questions[index].turns_until_repeat =
                rng.gen_range(20..40 * questions[index].corrects - questions[index].wrongs);
        } else {
